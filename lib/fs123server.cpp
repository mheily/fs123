...
void dup_suppress_log(int severity, const char *msg) {
    static thread_local std::string prevmsg;
    static thread_local unsigned long prevcount{0};
    if (msg == prevmsg) {
	if (prevcount == 0) {
	    prevcount++;
...
void validate_path(str_view pi){
    ...
    // We could also reject paths that end with / and paths that
    // contain //.  Such paths are never generated by the fuse client,
    // but they might be generated by external "tools".  They seem
    // unlikely to be malicious, so we let them through and leave it
    // up to the handler whether to accept or reject.
...
req::parse_and_handle(req::up req) try {
    ...
    // The /p passthru function is special.  We try to leave it alone
    // as much as possible.  E.g., it can have methods other than GET
    // and HEAD.  It can have a path_info that would be 'invalid' for
    // fs123.  It might use its own encryption scheme (e.g., munge).
    // The reply message body doesn't contain kvpairs.  Etc.
    if(req->function != "p"){
        switch (req->method) {
        case fs123p7::GET:
        case fs123p7::HEAD:
        ...
        // Copy the fs123-specific part of the original uri to the kvpairs:
        // evhttp_request_get_uri returns a char* to the unmodified, original uri.
        // This is easier to work with than the broken-down evhttp_uri.
        const char* original_uri = evhttp_request_get_uri(req->evhr);
        const char* where = ::strstr(original_uri, "/fs123/");
        if(where == nullptr)
            httpthrow(500, "No SIGIL?  Didn't we check this already?");
        req->kvpairs.emplace_back(FS123_REQUEST, where);
    }
    ...
    if(req->function == "a"){
        ...
        try{
            if(req->proto_minor < 3){
                ...
                if(begin){
                    // should we care what follows the 'begin'?  I.e., do we really care enough to
                    // return a 400 for .../d?128;1;xyzzy, or is it ok to just pass
                    // this on to handler::d(..., start="");
                    start_offset = req->query.size(); // start will be empty, as will be the unescaped string
                }else
                    start_offset = from+1;            // +1 skip semicolon (or other separator)
            }else{
                // The query is Len;Start
                // Len is an unsigned integer number of kilobytes
                // Start is a url-escaped string
                start_offset = svscan(req->query, &lenkib, 0);
                // Insist on either a semi-colon or a comma  after the number.  Since what follows is a
                // string, we're a little more picky that it be "puncutated" correctly.
                switch(req->query.at(start_offset)){
                case ';':
                case ',':
                    start_offset += 1;
                    break;
                default:
                    throw std::runtime_error("expected a semicolon or comma in query string");
                }
            }
        }catch(std::exception& e){
        ...
        if(lenkib > max_reply_size/1024)
            httpthrow(400, "/d requested length too large: " + std::to_string(lenkib) + " > " + std::to_string(max_reply_size));
        auto requested_len = lenkib*1024;
        auto start = req->query.substr(start_offset);
        req->allocate_pbuf(requested_len);
        handler.d(std::move(req), inm64, urlunescape(start));
        ...
        if(lenkib > max_reply_size/1024)
            httpthrow(400, "/f requested length too large: " + std::to_string(lenkib) + " > " + std::to_string(max_reply_size));
        auto requested_len = lenkib*1024;
        ...
        if(offsetkib > std::numeric_limits<uint64_t>::max()/1024)
            httpthrow(400, "/f requested offset overflows size_t: " + std::to_string(offsetkib));
        ...
        if(lenkib > max_reply_size/1024)
            httpthrow(400, "/x requested length too large: " + std::to_string(lenkib) + " > " + std::to_string(max_reply_size));
        auto requested_len = lenkib*1024;
...
void /* private */
req::common_reply200(const std::string& cc, uint64_t etag64/*=0*/){
    if(proto_minor >= 3){
        // Modify the buf (in-place!) so it looks like
        // a 7.3-style key-value pair.  I.e.,
        // Prepend: "6:content, <len>:"  and append ",\n"
        if(!blob)
            allocate_pbuf(0);
        buf = buf.prepend(netstring(FS123_CONTENT) + ' ' + std::to_string(buf.size()) + ':');
        buf = buf.append(",\n"); // final_netstring_bytes
        DIAG(_fs123server, "kvpairs.size(): " << kvpairs.size() << "\n");
        // prepend the other kvpairs.
        for(auto& p : kvpairs)
            buf = buf.prepend(netstring(p.first) + ' ' + netstring(p.second) + '\n');
    }else{
        // 7.2 *requires* an HHERRNO header.  errno_reply adds
        // one, but the other _reply functions don't.  So we check
        // before calling add_hdr.
        auto ohdrs = evhttp_request_get_output_headers(evhr);
        if(evhttp_find_header(ohdrs, HHERRNO)==nullptr)
            add_hdr(ohdrs, HHERRNO, "0");
    }
    encrypt_and_send200(cc, etag64);
}

void /* private */
req::encrypt_and_send200(const std::string& cc, uint64_t etag64){
    svr.incast_collapse_workaround(evhr);
    auto ohdrs = evhttp_request_get_output_headers(evhr);
    ...
    auto ob = evhttp_request_get_output_buffer(evhr);
    core123::threeroe tr;
    ...
    handler(h)
{
    ...
    if(gopts->sharedkeydir){
        acfd fd = sew::open(gopts->sharedkeydir->c_str(), O_DIRECTORY|O_RDONLY);
        the_secret_manager.emplace(std::move(fd), gopts->encoding_keyid_file, gopts->sharedkeydir_refresh);
    }
    ...
    if(gopts->exit_after_idle){
        idle_timeout = std::chrono::duration_cast<std::chrono::system_clock::duration>(std::chrono::duration<double>(gopts->exit_after_idle.value()));
    }else{
    ...
    DIAGf(_fs123server, "evhttp_bind_socket_with_handle(%p, %s, %d)",
          ehac.get(), gopts->bindaddr.c_str(), gopts->port.value_or(0));
    ehsock = evhttp_bind_socket_with_handle(ehac, gopts->bindaddr.c_str(), gopts->port.value_or(0));
    if (ehsock == nullptr)
...
// The xxx_reply functions MUST NOT throw.  If they do, then the standard idiom
// for writing a handler would be badly broken:
//
// handler::f(up req, ...) try {
//   ...
//   return some_flavor_of_reply(move(req), ...); // if this throws
//   ...
// }catch(exception& e){
//   exception_reply(move(req), e);  // then this segfaults!
// }
void req::errno_reply(int eno, const std::string& cc) try {
    DIAGf(_fs123server, "errno_reply(eno=%d, cc=%s)\n", eno, cc.c_str());
    if(proto_minor >= 3){
        kvpairs.emplace_back(FS123_ERRNO, std::to_string(eno));
    }else{
        auto ohdrs = evhttp_request_get_output_headers(evhr);
        add_hdr(ohdrs, HHERRNO, std::to_string(eno));
    }        
    common_reply200(cc, 0);
 }catch(std::exception& e){ internal_exception(e); }

void req::not_modified_reply(const std::string& cc) try {
    if(inm.empty())
        return internal_exception(http_exception(500, "handler called not_modified_reply but there is no If-None-Match header"));
    auto ohdrs = evhttp_request_get_output_headers(evhr);
...
void req::redirect_reply(const std::string& location, const std::string& cc) try {
    auto ohdrs = evhttp_request_get_output_headers(evhr);
...
void req::a_reply(const struct stat& sb, uint64_t validator, uint64_t esc, const std::string& cc) try {
    ...
    if(proto_minor >= 3){
        kvpairs.emplace_back(FS123_COOKIE, std::to_string(esc));
        kvpairs.emplace_back(FS123_VALIDATOR, std::to_string(validator));
        copy_to_pbuf(str(sb));
    }else{
        auto evreq = evhr;
...
bool req::add_dirent(core123::str_view name, int type, uint64_t estale_cookie){
    if(function != "d")
    ...
    if(entry.size() > dirent_space_avail())
        return false;
...
bool req::add_dirent(const ::dirent& de, uint64_t estale_cookie){
    return add_dirent(de.d_name, de.d_type, estale_cookie);
}

size_t req::dirent_space_avail() const /*public*/{
...
void req::d_reply(const std::string& nextstart, uint64_t etag64, uint64_t esc, const std::string& cc) try {
        if(function != "d")
            httpthrow(500, "handler replied to " + std::string(function) + " with d_reply");

        if(buf.empty() && !nextstart.empty()){
            httpthrow(500, "empty dirents and not EOF.  Something is wrong");
        }
        if(proto_minor >= 3){
            kvpairs.emplace_back(FS123_COOKIE, std::to_string(esc));
            kvpairs.emplace_back(FS123_NEXTSTART, nextstart);
        }else{
            auto evreq = evhr;
            auto ohdrs = evhttp_request_get_output_headers(evreq);
            add_hdr(ohdrs, HHCOOKIE, std::to_string(esc));
            if(nextstart.empty()){
                add_hdr(ohdrs, HHNO, "0 EOF");
            }else{
                try{
                    (void)svto<uint64_t>(nextstart);
                }catch(std::exception& e){
                    std::throw_with_nested(http_exception(500, "non-numeric nextstart (" + nextstart + ") in reply to /7/2/d request"));
                }
                add_hdr(ohdrs, HHNO, nextstart);
            }
        }
...
void req::f_reply(size_t nread, uint64_t content_validator, uint64_t etag64, uint64_t esc, const std::string& cc) try {
        ...
        if(proto_minor >= 3){
            kvpairs.emplace_back(FS123_COOKIE, std::to_string(esc));
            kvpairs.emplace_back(FS123_VALIDATOR, std::to_string(content_validator));
        }else{
            auto evreq = evhr;
            auto ohdrs = evhttp_request_get_output_headers(evreq);
            add_hdr(ohdrs, HHCOOKIE, std::to_string(esc));
            DIAGf(_fs123server, "prepend content validator (%llu) to buf.  buf_content_avail_front() = %zd, buf_content_avail_back()=%zd",
                  (unsigned long long)content_validator, buf_content_avail_front(), buf_content_avail_back());
...
void req::p_reply(const std::string& body, uint64_t etag64, const std::string& cc) try {
        ...
        copy_to_pbuf(body);
        encrypt_and_send200(cc, etag64); // Just the body.  No fs123-style key/value pairs!
 }catch(std::exception& e) { internal_exception(e); }
